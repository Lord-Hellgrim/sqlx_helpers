...
pub fn read_to_hashmap(path: &Path, sep: char, pk: usize) -> (Vec<String>, HashMap<String, HashMap<String, String>>) {
...
mod tests {
    ...
    #[test]
    fn test_read_to_hashmap() {
        let path = Path::new("sample_data.txt");
        let (head, result2) = read_to_hashmap(&path, '\t', 0);
        println!("Header: {:?}", head);
    ...
    fn pk_bigger_than_header() {
        let path = Path::new("sample_data.txt");
        let (head, result2) = read_to_hashmap(&path, '\t', 500);
